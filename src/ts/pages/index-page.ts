...
function getForm(id: string): HTMLFormElement | null {
  const element = getElement(id);
...
const redirectToPortal = (email: string): void => {
...
function openPhotoModal(src: string, alt: string): void {
...
function showSlide(index: number): void {
  ...
  const dots = Array.from(document.querySelectorAll<HTMLElement>(".indicator-dot"));
...
function applyHomepageContent(): void {
  ...
  Array.from(carouselTrack.querySelectorAll<HTMLElement>("[data-carousel-image]")).forEach(
    (image) => {
  ...
  Array.from(carouselIndicators.querySelectorAll<HTMLButtonElement>("[data-slide-index]")).forEach(
    (button) => {
...
Array.from(document.querySelectorAll<HTMLAnchorElement>("#mobileMenu a")).forEach((link) => {
